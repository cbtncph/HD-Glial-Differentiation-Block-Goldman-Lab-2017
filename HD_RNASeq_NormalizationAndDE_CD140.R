...
# HD_RNASeq_NormalizationAndDE_CD140.R
...
# 3 - second-pass differential expression analysis (1% FDR and log2 fold change > 1) for
...
experiment_title = "HD Differentiation Delay - CD140a+ GPCs"

### read counts and sample data sheet
countData_allSamples  <- read.table("countData_GeneaHD_CD140.txt", header = TRUE, sep = "\t", row.names = 1)
sampleData_allSamples <- read.table("sampleData_GeneaHD_CD140.txt", header = TRUE, sep = "\t", row.names = 1)

### filter out mis-clustered samples:
countData_allSamples <- countData_allSamples[,c(1:9,11,14:22,24:26,28)]
sampleData_allSamples <- sampleData_allSamples[c(1:9,11,14:22,24:26,28),]
...
for (which_comparison in 1:4) {
  ...
  if (which_comparison == 1) {
    ...
    suffix <- " - HD17vCTR_CD140"
    
    ### subset countData:
    countData <- countData_allSamples[,c(1:10, 11:14)]
    
    ### subset sampleData:
    sampleData <- sampleData_allSamples[c(1:10, 11:14),]
    ...
    x <- factor(c(rep(1, 10), rep(2, 4)))
    
  } else if (which_comparison == 2) {
    
    suffix <- " - HD18vCTR_CD140"
    countData <- countData_allSamples[,c(1:10, 15:19)]
    sampleData <- sampleData_allSamples[c(1:10, 15:19),]
    cell_line_colors <- c(col_02, col_19, col_18)
    x <- factor(c(rep(1, 10), rep(2, 5)))
    
  } else if (which_comparison == 3) {
    
    suffix <- " - HD20vCTR_CD140"
    countData <- countData_allSamples[,c(1:10, 20:23)]
    sampleData <- sampleData_allSamples[c(1:10, 20:23),]
    cell_line_colors <- c(col_02, col_19, col_20)
    x <- factor(c(rep(1, 10), rep(2, 4)))
    
  } else if (which_comparison == 4) {
    
    suffix <- " - HD20vCTR19_CD140"
    countData <- countData_allSamples[,c(7:10, 20:23)]
    sampleData <- sampleData_allSamples[c(7:10, 20:23),]
...
degs_001_17vsCTR <- read.table("Output - HD17vCTR_CD140/Output - DE Second Pass/DEGs_INTs/DEGs_001 - W_1.txt", sep = "\t")
degs_001_18vsCTR <- read.table("Output - HD18vCTR_CD140/Output - DE Second Pass/DEGs_INTs/DEGs_001 - W_1.txt", sep = "\t")
degs_001_20vsCTR <- read.table("Output - HD20vCTR_CD140/Output - DE Second Pass/DEGs_INTs/DEGs_001 - W_1.txt", sep = "\t")
degs_001_20vs19  <- read.table("Output - HD20vCTR19_CD140/Output - DE Second Pass/DEGs_INTs/DEGs_001 - W_1.txt", sep = "\t")


degs_001_fc2_17vsCTR_up <- degs_001_17vsCTR[which(degs_001_17vsCTR$logFC >  1),]
degs_001_fc2_17vsCTR_dn <- degs_001_17vsCTR[which(degs_001_17vsCTR$logFC < -1),]

degs_001_fc2_18vsCTR_up <- degs_001_18vsCTR[which(degs_001_18vsCTR$logFC >  1),]
degs_001_fc2_18vsCTR_dn <- degs_001_18vsCTR[which(degs_001_18vsCTR$logFC < -1),]

degs_001_fc2_20vsCTR_up <- degs_001_20vsCTR[which(degs_001_20vsCTR$logFC >  1),]
degs_001_fc2_20vsCTR_dn <- degs_001_20vsCTR[which(degs_001_20vsCTR$logFC < -1),]

degs_001_fc2_20vs19_up <- degs_001_20vs19[which(degs_001_20vs19$logFC >  1),]
degs_001_fc2_20vs19_dn <- degs_001_20vs19[which(degs_001_20vs19$logFC < -1),]


output_folder <- "Output - DE Genes Intersections - CD140/"
dir.create(output_folder)

labels <- c("17vsCTR","18vsCTR","20vsCTR","20vs19")


###### Gene set intersection and Venn diagram for up-regulated genes, 1% FDR and FC > 2:

l1 <- rownames(degs_001_fc2_17vsCTR_up)
l2 <- rownames(degs_001_fc2_18vsCTR_up)
l3 <- rownames(degs_001_fc2_20vsCTR_up)
l4 <- rownames(degs_001_fc2_20vs19_up)

png(paste(output_folder, "Venn 001_FC2 - 4 Lists - UP.png", sep = ""), width = 800, height = 700, units = "px")  
...
degs_int_001_fc2_up <- cbind(degs_001_fc2_17vsCTR_up[Reduce(intersect, list(v1 = l1, v2 = l2, v3 = l3, v4 = l4)),c(7,11)],
                             degs_001_fc2_18vsCTR_up[Reduce(intersect, list(v1 = l1, v2 = l2, v3 = l3, v4 = l4)),c(7,11)],
                             degs_001_fc2_20vsCTR_up[Reduce(intersect, list(v1 = l1, v2 = l2, v3 = l3, v4 = l4)),c(7,11)],
                             degs_001_fc2_20vs19_up[Reduce(intersect, list(v1 = l1, v2 = l2, v3 = l3, v4 = l4)),c(7,11)])

###### Gene set intersection and Venn diagram for down-regulated genes, 1% FDR and FC > 2:

l1 <- rownames(degs_001_fc2_17vsCTR_dn)
l2 <- rownames(degs_001_fc2_18vsCTR_dn)
l3 <- rownames(degs_001_fc2_20vsCTR_dn)
l4 <- rownames(degs_001_fc2_20vs19_dn)

png(paste(output_folder, "Venn 001_FC2 - 4 Lists - DN.png", sep = ""), width = 800, height = 700, units = "px")  
vd <- venn.diagram(list(v1 = l1, v2 = l2, v3 = l3, v4 = l4),
...
### compile the intersection gene list with fold changes and P Values calculated by edgeR:
degs_int_001_fc2_dn <- cbind(degs_001_fc2_17vsCTR_dn[Reduce(intersect, list(v1 = l1, v2 = l2, v3 = l3, v4 = l4)),c(7,11)],
                             degs_001_fc2_18vsCTR_dn[Reduce(intersect, list(v1 = l1, v2 = l2, v3 = l3, v4 = l4)),c(7,11)],
                             degs_001_fc2_20vsCTR_dn[Reduce(intersect, list(v1 = l1, v2 = l2, v3 = l3, v4 = l4)),c(7,11)],
                             degs_001_fc2_20vs19_dn[Reduce(intersect, list(v1 = l1, v2 = l2, v3 = l3, v4 = l4)),c(7,11)])


degs_int_001_fc2 <- rbind(degs_int_001_fc2_up, degs_int_001_fc2_dn)

colnames(degs_int_001_fc2) <- c("logFC_HD17vsCTR","FDR_HD17vsCTR",
...
write.table(degs_int_001_fc2, paste(output_folder, "DEGs 4 Lists - FDR001_FC2.txt", sep = ""), sep = "\t", quote = FALSE)
